...
def test_dataset_local_source_manifest_is_found_via_generation_source_root(tmp_path):
    patch_root = tmp_path / "vascular_patches" / "policy"
    source_root = tmp_path / "vascular_graph_sources" / "policy"
    (patch_root / "val" / "vtp").mkdir(parents=True)
    source_root.mkdir(parents=True)
    source = tmp_path / "source.nii.gz"
    affine = np.diag([.5, .6, 1.2, 1.])
    nib.save(nib.Nifti1Image(np.zeros((2, 2, 2)), affine), source)
    (patch_root / "generation_config.json").write_text(json.dumps({
        "patch_size": [64, 64, 64], "pad": [0, 0, 0],
        "source_root": str(source_root),
    }))
    (source_root / "source_manifest.json").write_text(json.dumps({
        "subjects": [{"patient_id": "000012", "image": str(source)}],
    }))
    with (patch_root / "patch_index.csv").open("w", newline="") as stream:
        writer = csv.DictWriter(stream, fieldnames=[
            "sample_id", "patient_id", "start_d", "start_h", "start_w",
        ])
        writer.writeheader()
        writer.writerow({"sample_id": "sample_000012_0062", "patient_id": "000012",
                         "start_d": 5, "start_h": 7, "start_w": 9})
    record = SimpleNamespace(sample_id="sample_000012_0062",
                             graph=patch_root / "val" / "vtp" / "sample_000012_0062_graph.vtp")
    transform = PatchPhysicalCoordinates().transform(record)
    np.testing.assert_allclose(to_world_mm([[.5, .5, .5]], transform)[0],
                               (affine @ [37, 39, 41, 1])[:3])
