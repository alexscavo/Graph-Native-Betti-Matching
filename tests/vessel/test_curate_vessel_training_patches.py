...
def test_moves_only_empty_training_samples_and_keeps_full_grid_index(tmp_path: Path):
    ...
    all_splits = curate(root, apply=True, all_splits=True)
    assert all_splits["excluded_patches_by_split"] == {"train": 2, "test": 1}
    assert all_splits["active_patches_by_split"] == {"train": 1}
    assert not (root / "test" / "raw" / "sample_000001_0000_data.nii.gz").exists()
    assert (root / "excluded_test" / "raw" / "sample_000001_0000_data.nii.gz").is_file()
    assert (root / "patch_index.csv").read_bytes() == before
    assert curate(root, apply=True, all_splits=True) == all_splits
